...
read -p "Enter your choice: " file_choice
        ...
        case $file_choice in
            1) search_file; echo; fileManMenu ;;
            2) largest_files; echo; fileManMenu ;;
            3) oldest_files; echo; fileManMenu ;;
            4) email_file; echo; fileManMenu ;;
...
#makes sure if the inserted date is not in the past
...
#creating this just to avoid multiple backup scripts with the same time / it is otherwise useless in terms of backuping
second=`date +"%S"`


#create a backup script that the cron job will use
...
#changing perms so that the cronjob can executed it
...
## Service management
# Changes the #? to whatever you want to make it more user friendly
